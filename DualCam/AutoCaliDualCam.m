...
for idx = 1:size(img_list1,1)
    ...
    if size(im_corners1,2)<4||size(im_corners2,2)<4
        disp("Frame: "+num2str(idx)+"st, droped");
        continue;
    end
    cam_corners3D1{num} = im_corners1;
    cam_corners3D2{num} = im_corners2;
    cam_bors_coeff1{num} = cam_plane_coeff1;
    cam_bors_coeff2{num} = cam_plane_coeff2;
    num = num +1;
end
%% optimize
...
disp("********The final extrinsics: camera 2 to camera 1********")
disp(TOptm);
