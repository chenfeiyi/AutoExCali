% pc_raw = pcread("/home/cfy/Documents/unifiedCali/data/simu/cam-lidar/noise-free/pcd/2112.629000.pcd");
% pc_array = pc_raw.Location()';
% pts_out = f_bor_edge_noise2(pc_array,0.77,0.63);
% pc_all = [pc_array,pts_out];
% 
% pc_save = pointCloud(pc_all');
% pc_save.Intensity = [zeros([size(pc_array,2),1]);255*ones([size(pts_out,2),1])];
% pcwrite(pc_save,"test.pcd");

% figure;
% plot3(pc_array(1,:),pc_array(2,:),pc_array(3,:),".r");
% hold on;
% plot3(pts_out(1,:),pts_out(2,:),pts_out(3,:),"*b");
% axis equal;
