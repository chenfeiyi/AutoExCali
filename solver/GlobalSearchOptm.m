...
x0 = [0,0,0,0,0,0]; % roll pitch yaw,tx,ty,tz
TOptm = eye(4);
%% optimize
...
FAll = @(x) object_func(x(1:3),x(4:6),tf0,edges_pts,corners3D);
...
problem = createOptimProblem('fmincon','objective',FAll,'x0',x0,'lb',4*[-0.02,-0.02,-0.02,-0.1,-0.1,-0.1],...
    'ub',4*[0.02,0.02,0.02,0.1,0.1,0.1],'options',opts);
gs = GlobalSearch;
[x,f] = run(gs,problem);
R = eul2rotm(x(1:3));
TOptm(1:3,1:3) = R;
...
function error = object_func(rvec,tvec,TInit,edges_pts,corners)
R = eul2rotm(rvec);
error=0;
